...
/**
 * Class that extends ValueMeta to add a few fields specific to HBase and a constructor that allows a fully qualified
 * name (i.e column_family,column_name).
 * <p/>
 * Note that for ordinary (non-key) columns in HBase, dates are assumed to be just raw long values. Filtering on dates
 * involves using a WritableByteArrayComparable comparator in order to first deserialize the long before performing any
 * comparison. This differs from the key of the table where we allow either raw longs or longs where the sign bit has
 * been flipped (so that the natural sort order is preserved by HBases's byte-wise comparison).
 *
...
public class HBaseValueMeta extends ValueMeta {
  ...
  /**
   * In HBase, for filtering on unsigned columns, we need to know if a number is double/long or float/int in order to
   * convert the comparison constant to the right number of bytes for a lexical comparison to work properly
   */
  ...
  /**
   * Set whether this field is a long integer or a double precision floating point number
   *
   * @param ld true if this field is either a long integer or a double precision floating point number
   */
  ...
  /**
   * Returns true if this field is either a long integer or double precision floating point number
   *
   * @return true if this field is a long integer or double precision floating point number
   */
  ...
  /**
   * Encode a key value (object) to an array of bytes with respect to the key type specified in a mapping.
   *
   * @param keyValue the key value (object) to encode
  ...
  /**
   * Encode a key value (string) to an array of bytes. Useful for encoding values entered by the user through the UI for
  ...
  /**
   * Decode/deserialize a big decimal. Tries the raw value as a string first. If this fails then it tries to decode the
   * big decimal as a serialized object.
   *
  ...
  /**
   * Utility method to convert a comma separated list of string values (for an indexed type) to an array of objects.
   *
   * @param list the comma separated list of values to parse
   * @return an array of objects where each element contains one value from the list
  ...
  /**
   * Utility method to convert an array of objects containing indexed values to a comma separated list as a string.
   *
