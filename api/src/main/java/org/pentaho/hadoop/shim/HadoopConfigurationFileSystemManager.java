...
public class HadoopConfigurationFileSystemManager {
  ...
  public HadoopConfigurationFileSystemManager( HadoopConfigurationProvider configProvider,
                                               DefaultFileSystemManager delegate ) {
  ...
  public FileProvider getFileProvider( HadoopConfiguration config, String scheme ) throws FileSystemException {
    ...
    if ( p == null ) {
      throw new FileSystemException(
        BaseMessages.getString( PKG, "Error.UnsupportedSchemeForConfiguration", config.getName(), scheme ) );
    }
    return p;
  }
  ...
  /**
   * Delegates to the underlying file system manager to determine if the scheme has been registered.
   *
