...
/**
 * A non-thread safe mechanism to execute code against a given JDBC Driver. The code executed within {@link
 * #callWithDriver(Driver)} will have the current Thread's context ClassLoader set to the driver's.
 *
 * @param <T> Type of value to return from {@link #call()}
 */
public abstract class JDBCDriverCallable<T> implements Callable<T> {
  /**
   * Cached driver value passed in via {@link #callWithDriver(Driver)} that may be accessed within {@link #call()}.
   */
  protected Driver driver;

  /**
   * Sets the current thread's context class loader to the driver's class loader for the invocation of {@link #call()}.
   *
