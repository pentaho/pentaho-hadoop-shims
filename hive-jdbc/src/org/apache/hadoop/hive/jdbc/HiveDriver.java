...
/**
 * <p> This is proxy driver for the Hive JDBC Driver available through the current active Hadoop configuration. </p> <p>
 * This driver is named exactly the same as the official Apache Hive driver so no further modifications are required by
 * calling code to swap in this proxy. </p> <p> This class uses reflection to attempt to find the Big Data Plugin and
...
public class HiveDriver implements java.sql.Driver {
   ...
   * appropriate Hive JDBC driver with all associated Hadoop and other libraries required to properly execute against a
   * given cluster.
   *
