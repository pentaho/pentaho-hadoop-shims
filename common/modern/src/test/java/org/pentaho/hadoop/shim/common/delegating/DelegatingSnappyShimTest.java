...
/**
 * User: Dzmitry Stsiapanau Date: 02/18/2016 Time: 13:57
 */

public class DelegatingSnappyShimTest {
  private DelegatingSnappyShim dhs;
  private SnappyShim shimMock;
  private HadoopAuthorizationService has;

  @org.junit.Test
  public void testCheckAllMethodsDelegated() throws Exception {
    dhs = new DelegatingSnappyShim();
    has = mock( HadoopAuthorizationService.class );
    final AtomicBoolean called = new AtomicBoolean( false );
    shimMock = mock( SnappyShim.class, new Answer() {
      @Override public Object answer( InvocationOnMock invocation ) throws Throwable {
        called.set( true );
        return null;
      }
    } );
    when( has.getShim( SnappyShim.class ) ).thenReturn( shimMock );
    dhs.setHadoopAuthorizationService( has );
    Method[] methods = SnappyShim.class.getDeclaredMethods();
