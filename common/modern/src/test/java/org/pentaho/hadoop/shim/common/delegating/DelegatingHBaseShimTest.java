...
public class DelegatingHBaseShimTest {
  private DelegatingHBaseShim dhs;
  private HBaseShimInterface shimMock;
  private HadoopAuthorizationService has;

  @org.junit.Test
  public void testCheckAllMethodsDelegated() throws Exception {
    dhs = new DelegatingHBaseShim();
    has = mock( HadoopAuthorizationService.class );
    final AtomicBoolean called = new AtomicBoolean( false );
    shimMock = mock( HBaseShimInterface.class, new Answer() {
      @Override public Object answer( InvocationOnMock invocation ) throws Throwable {
        called.set( true );
        return null;
      }
    } );
    when( has.getShim( HBaseShimInterface.class ) ).thenReturn( shimMock );
    dhs.setHadoopAuthorizationService( has );
    Method[] methods = HBaseShimInterface.class.getDeclaredMethods();
