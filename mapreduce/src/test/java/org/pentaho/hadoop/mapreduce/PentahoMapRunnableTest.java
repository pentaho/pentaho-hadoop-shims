...
public class PentahoMapRunnableTest {
  ...
  public void testMapper_null_output_value() throws Exception {
    ...
    Assert.assertEquals( "Received output when we didn't expect any.  <null>s aren't passed through.", 0,
  ...
  public void testLogChannelLeaking() throws Exception {
    ...
    Assert.assertEquals( "Incorrect output ", expectedOutputCollectorMockSize, outputCollectorMock.getCollection().size() );
    ...
    for ( int i = RUNS; i > 0; i-- ) {
      expectedWordCountArrays = IntStream.rangeClosed( 1, RUNS - i + 1 ).mapToObj( value -> new IntWritable( 1 ) )
        .collect( Collectors.toList() );
      Assert.assertEquals( "Incorrect count array for the word: " + WORD_TO_COUNT_TEMPLATE + i, expectedWordCountArrays,
