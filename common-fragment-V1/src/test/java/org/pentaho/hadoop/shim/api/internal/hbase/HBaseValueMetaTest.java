...
public class HBaseValueMetaTest extends HBaseValueMeta {
  ...
  /**
   * the byte array with size not covered test methods in {@link MockHBaseBytesUtilShim} that allows to get size of the byte arrays.
   * E.g. {@link MockHBaseBytesUtilShim#getSizeOfShort()}
   */
  ...
  public void testDecodeColumnValueStringIndexedStorageIllegalValue() {
    ...
    HBaseValueMeta mappingMeta = new HBaseValueMeta( "famliy1" + SEPARATOR + "testcol" + SEPARATOR + "anAlias", TYPE_STRING, -1, -1 );
  ...
  public void testDecodeColumnValue() throws Exception {
    ...
    keyValue = (short) 6;
    assertEquals( Long.valueOf( keyValue.toString() ), decodeColumnValue( new byte[] { 1, 1, 1, 1, 1 }, hbMeta, BYTES_UTIL ) );
    hbMeta.setIsLongOrDouble( true );
    keyValue = 2L;
