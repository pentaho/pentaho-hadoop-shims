...
public class PropertiesConfigurationProperties extends Properties {
  private transient ReloadingFileBasedConfigurationBuilder<PropertiesConfiguration> configurationBuilder;
  ...
  private static ReloadingFileBasedConfigurationBuilder<PropertiesConfiguration> initConfigurationBuilder(
    FileObject fileObject ) throws FileSystemException, ConfigurationException {
  ...
  private PropertiesConfiguration getPropertiesConfiguration() {
    ...
    try {
      // checkForReloading() is cheap on hot paths: the builder's FileHandlerReloadingDetector only performs an actual
      // filesystem stat once per its refresh delay (commons-configuration2 default, ~5s). Between checks it is a simple
      // timestamp comparison, so calling it per property access does not add frequent filesystem I/O.
