...
public class AuthenticatingHadoopShim extends DelegatingHadoopShim {
  ...
  public void onLoad( HadoopConfiguration config, HadoopConfigurationFileSystemManager fsm ) throws Exception {
    ...
    if ( config.getConfigProperties().containsKey( DelegatingHadoopShim.SUPER_USER ) && !config.getConfigProperties()
      .getProperty( MAPPING_IMPERSONATION_TYPE, "" ).trim().equalsIgnoreCase( "disabled" ) ) {
      provider = config.getConfigProperties().getProperty( DelegatingHadoopShim.SUPER_USER );
    ...
    new PropertyAuthenticationProviderParser( config.getConfigProperties(), manager ).process( DelegatingHadoopShim.PROVIDER_LIST );
    ...
    if ( performer == null ) {
      throw new RuntimeException( "Unable to find relevant provider for chosen authentication method (id of "
        + config.getConfigProperties().getProperty( DelegatingHadoopShim.SUPER_USER ) );
    } else {
