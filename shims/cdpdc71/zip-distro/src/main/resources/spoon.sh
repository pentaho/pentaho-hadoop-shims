...
case `uname -s` in 
	...
	ARCH=`uname -m`
		case $ARCH in
			...
			ppc)
				LIBPATH=$CURRENTDIR/../libswt/aix/:$CURRENTDIR/../native-lib/aix/:$CURRENTDIR/native-lib/aix/
				;;

			ppc64)
				LIBPATH=$CURRENTDIR/../libswt/aix64/:$CURRENTDIR/../native-lib/aix64/:$CURRENTDIR/native-lib/aix64/
				;;
	...
	ARCH=`uname -m`
		case $ARCH in
			...
			i[3-6]86)
				LIBPATH=$CURRENTDIR/../libswt/solaris-x86/:$CURRENTDIR/../native-lib/solaris-x86/:$CURRENTDIR/native-lib/solaris-x86/
				;;

			*)	
				LIBPATH=$CURRENTDIR/../libswt/solaris/:$CURRENTDIR/../native-lib/solaris/:$CURRENTDIR/native-lib/solaris/
				;;
		esac
		;;
	...
	Linux)
		...
		case $ARCH in
			x86_64)
				if $($_PENTAHO_JAVA -version 2>&1 | grep "64-Bit" > /dev/null )
                                then
				  LIBPATH=$CURRENTDIR/../libswt/linux/x86_64/:$CURRENTDIR/../native-lib/linux/x86_64/:$CURRENTDIR/native-lib/linux/x86_64/
                                else
				  LIBPATH=$CURRENTDIR/../libswt/linux/x86/:$CURRENTDIR/../native-lib/linux/x86/:$CURRENTDIR/native-lib/linux/x86/
                                fi
				;;
			...
			ppc)
				LIBPATH=$CURRENTDIR/../libswt/linux/ppc/:$CURRENTDIR/../native-lib/linux/ppc/:$CURRENTDIR/native-lib/linux/ppc/
				;;

			ppc64)
				LIBPATH=$CURRENTDIR/../libswt/linux/ppc64/:$CURRENTDIR/../native-lib/linux/ppc64/:$CURRENTDIR/native-lib/linux/ppc64/
				;;
	...
	HP-UX) 
		LIBPATH=$CURRENTDIR/../libswt/hpux/:$CURRENTDIR/../native-lib/hpux/:$CURRENTDIR/native-lib/hpux/
		;;
	CYGWIN*)
		./Spoon.bat
		exit
...
# Remove old webservices libraries
if [ -f $BASEDIR/lib/webservices-api-2.3.1.jar ] ; then
    echo "Removing old webservices-api libraries from lib folder..."
    rm -f $BASEDIR/lib/webservices-api-2.3.1.jar
fi
if [ -f $BASEDIR/lib/webservices-rt-2.3.1.jar ] ; then
    echo "Removing old webservices-rt libraries from lib folder..."
    rm -f $BASEDIR/lib/webservices-rt-2.3.1.jar
fi
